  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor: Colors.black, appBar: AppBar(backgroundColor: Colors.black, leading: IconButton(icon: const Icon(Icons.keyboard_arrow_down, color:Colors.white), onPressed:()=>Navigator.pop(context)), title: Text(p.queue.length>1?'Antrian • ${p.queue.length}':'Musera', style:const TextStyle(color:Colors.white, fontSize:12)), centerTitle:true, actions:[IconButton(icon: const Icon(Icons.more_vert, color:Colors.white), onPressed:()=>_showAddPlaylistSheet(Map<String,dynamic>.from(song)))],),
      body: SingleChildScrollView(padding: const EdgeInsets.symmetric(horizontal:24, vertical:8), child: Column(children:[
        ...
        if(p.error!=null) Container(margin: const EdgeInsets.only(top:8), padding: const EdgeInsets.symmetric(horizontal:12, vertical:8), decoration: BoxDecoration(color: Colors.redAccent.withOpacity(0.12), borderRadius: BorderRadius.circular(8)), child: Row(children:[Expanded(child: Text(p.error!, style:const TextStyle(color:Colors.redAccent, fontSize:12))), const SizedBox(width:8), if(p.error!.contains('502')||p.error!.contains('503')||p.error!.contains('Coba lagi')) FilledButton(onPressed:()=>ref.read(playerProvider.notifier).retry(), style: FilledButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(horizontal:12, vertical:6), minimumSize: Size.zero), child: const Text('Coba lagi', style:TextStyle(fontSize:11, fontWeight:FontWeight.w800))) else if(p.error!.contains('404')||p.error!.contains('tidak tersedia')) FilledButton(onPressed:()=>ref.read(playerProvider.notifier).skipUnavailable(), style: FilledButton.styleFrom(backgroundColor: Colors.white, foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(horizontal:12, vertical:6), minimumSize: Size.zero), child: const Text('Skip', style:TextStyle(fontSize:11, fontWeight:FontWeight.w800)))] )),
